...
class TestCostFunctions(unittest.TestCase):
    def test_adapt_lms(self):
        ...
        self.assertEqual(test_function.original(0.1, 0.1), 0.0)
        self.assertEqual(test_function.original(0.1, 0.6), 0.5 ** 2)
        # derivative
    ...
    def test_adapt_lmls(self):
        ...
        self.assertEqual(test_function.original(0.1, 0.1), 0.0)
        self.assertEqual(test_function.original(0.1, 0.6), log(1 + 0.5 ** 2 / 2))
        # derivative
        self.assertEqual(test_function.derivative(0.1, 0.1), 0.0)
...
"""
    def test_batch_lms(self):
        ...
        self.assertEqual(test_function.original(output, output), 0.0)
        self.assertEqual(test_function.original(output, target), np.mean((output - target) ** 2))
        # derivative
